...
const RegQueueAdharModal = ({isAdharModal, onClose, data}) => {
  return (
    <Modal
      isVisible={isAdharModal}
      ...
      <View style={styles.modalContainer}>
        ...
        {data?.isStudentReview && (
          <View style={{gap: verticalScale(12)}}>
            ...
            <Card>
              <Text style={styles.text}>{`Name : ${data?.data[0]?.name}`}</Text>
              <Text
                style={styles.text}>{`Mobile : ${data?.data[0]?.mobile}`}</Text>
              <Text
...
export default RegQueueAdharModal;
